...
object EntityLerper {
    ...
    fun lerpStep(dragInfo: EntityDraggingInformation, refship: Ship, entity: Entity) {
        ...
        if (dragInfo.lerpSteps > 0) {
            ...
            // Armor stands: skip the YAW lerp (the position lerp above is kept). The EntityDragger
            // carry ALONE then deck-locks the stand's yaw to the ship -- it rotates WITH the deck and
            // is still when parked (carry delta = 0). Running the lerp TOO made writer A fight the
            // carry + the periodic server re-sync, jittering the static-yaw body model.
