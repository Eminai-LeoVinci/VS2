...
object VSClientConfigLoader {
     ...
     * Best-effort: a write failure is logged, not thrown (the in-memory change still applies). Public for
     * the in-game border-tuning commands (when ported) to make live tuning stick across restarts.
     */
    @JvmStatic
    fun save() {
        try {
