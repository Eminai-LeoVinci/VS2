...
object ShipActivationManager {
        ...
        for (ship in shipWorld.loadedShips) {
            ...
            while (dvIter.hasNext()) {
                ...
                if (entry.voxelChunks.add(packed)) {
                    // 1.21.1: no addTicketWithRadius helper — use vanilla addRegionTicket (radius 0 =>
                    // level 33 FULL), the same call ChunkManagement uses for SHIP_CHUNK tickets.
                    val cp = ChunkPos(packed)
                    level.chunkSource.addRegionTicket(VSTicketType.SHIP_ACTIVE_VOXEL, cp, 0, cp)
                }
            }
            ...
            while (vIter.hasNext()) {
                ...
                if (!desiredVoxel.contains(packed)) {
                    val cp = ChunkPos(packed)
                    level.chunkSource.removeRegionTicket(VSTicketType.SHIP_ACTIVE_VOXEL, cp, 0, cp)
                    vIter.remove()
                }
            }
    ...
    private fun release(id: Long, server: MinecraftServer) {
        ...
        val vIter = entry.voxelChunks.iterator()
        while (vIter.hasNext()) {
            val cp = ChunkPos(vIter.nextLong())
            level.chunkSource.removeRegionTicket(VSTicketType.SHIP_ACTIVE_VOXEL, cp, 0, cp)
        }
    }
